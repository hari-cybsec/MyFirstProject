...
function supermart_ecommerce_customize_register( $wp_customize ) {
	...
	//front page
	$num_sections = apply_filters( 'supermart_ecommerce_front_page_sections', 4 );
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( __( 'Front Page Section %d Content', 'supermart-ecommerce' ), $i ),
			'description'    => ( 1 !== $i ? '' : __( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'supermart-ecommerce' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
			'active_callback' => 'supermart_ecommerce_is_static_front_page',
		) );

		$wp_customize->selective_refresh->add_partial( 'panel_' . $i, array(
			'selector'            => '#panel' . $i,
			'render_callback'     => 'supermart_ecommerce_front_page_section',
